    	...
    	<div class="row">
				<form method="get" action="ServletPrincipal">
					...
					<div class="col-md-6 col-sm-6 col-xs-6 text-center">
						<%if(session.getAttribute("minimo")!=null){ %>
								<div class="col-md-6 col-sm-6 col-xs-6">
									<span>Desde:</span> <input id="minimo" placeholder="$"
										type="number" min="0" name="minimo"
										value="<%=session.getAttribute("minimo")%>">
								</div>
						<%}else{%>
								<div class="col-md-6 col-sm-6 col-xs-6">
									<span>Desde:</span> <input id="minimo" placeholder="$"
						...
						<%}%>
						<%if(session.getAttribute("maximo")!=null){ %>
								<div class="col-md-6 col-sm-6 col-xs-6">
									<span>Hasta:</span> <input id="maximo" placeholder="$"
										type="number" min="0" name="maximo"
										value="<%=session.getAttribute("maximo")%>">
								</div>
						<%}else{%>
								<div class="col-md-6 col-sm-6 col-xs-6">
									<span>Hasta:</span> <input id="maximo" placeholder="$"
    		...
    		<c:if test="${NombreCategoria!=null}">
    			<h5>Categoría mostrada: ${NombreCategoria}</h5>
    		</c:if>
				...
				<ul class="pagination">
						...
						<c:choose>
							<c:when test="${Categoria !=null }">
								<li class="page-item"><a class="page-link"
									href="ServletPrincipal?paginaActual=${paginaActual-1}&Categoria=${Categoria}">Anterior</a>
								</li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="ServletPrincipal?paginaActual=${paginaActual-1}">Anterior</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:if>
						...
						<c:choose>
							...
							<c:otherwise>
								<c:choose>
									<c:when test="${Categoria !=null }">
										<li class="page-item"><a class="page-link"
											href="ServletPrincipal?paginaActual=${i}&Categoria=${Categoria}">${i}</a>
										</li>
									</c:when>
									<c:otherwise>
										<li class="page-item"><a class="page-link"
											href="ServletPrincipal?paginaActual=${i}">${i}</a>
										</li>
									</c:otherwise>
								</c:choose>
							</c:otherwise>
					...
					<c:if test="${paginaActual lt numeroDePaginas}">
						<c:choose>
							<c:when test="${Categoria !=null }">
								<li class="page-item"><a class="page-link"
									href="ServletPrincipal?paginaActual=${paginaActual+1}&Categoria=${Categoria}">Siguiente</a>
								</li>
							</c:when>
							<c:otherwise>
								<li class="page-item"><a class="page-link"
									href="ServletPrincipal?paginaActual=${paginaActual+1}">Siguiente</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:if>
